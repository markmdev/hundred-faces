...
import { REACTIONS, type Reaction } from "../src/shared/reactions.ts";
...
const tv = (a: FaceAnswer, b: FaceAnswer) => 0.5 * REACTIONS.reduce((s, r) => s + Math.abs(a.reaction[r] - b.reaction[r]), 0);
const mean = (xs: number[]) => xs.reduce((s, x) => s + x, 0) / xs.length;
const percentile = (xs: number[], p: number) => [...xs].sort((a, b) => a - b)[Math.min(xs.length - 1, Math.ceil((p / 100) * xs.length) - 1)]!;
const pct = (x: number) => `${Math.round(x * 100)}%`;
const mix = (faces: FaceAnswer[]) => Object.fromEntries(REACTIONS.map((r) => [r, pct(mean(faces.map((f) => f.reaction[r])))]));
...
console.log(`batch size ${BATCH_SIZE}, ${Math.ceil(PERSONAS.length / BATCH_SIZE)} calls per update\n`);
...
// (a) blunt versus gentle price increase
{
  ...
  console.log(`    faces whose top reaction changed: ${topChanged}/100; moved by TV > 0.2: ${moved.filter((m) => m > 0.2).length}/100; mean TV ${mean(moved).toFixed(3)}`);
  console.log(`    direction: ${towardsPositive} more favourable, ${towardsNegative} less favourable, ${100 - towardsPositive - towardsNegative} within 0.1`);
...
for (const id of ["blurb-jargon", "clickbait"]) {
  ...
  let sum = 0;
  let n = 0;
  for (let i = 0; i < 100; i++) for (let j = i + 1; j < 100; j++) { sum += tv(w.faces[i]!, w.faces[j]!); n++; }
  ...
  const describe = (i: number) => {
    const f = w.faces[i]!;
    return `${name(i)} (${topReaction(f.reaction)} ${pct(f.reaction[topReaction(f.reaction)])}, understands ${pct(f.understands)}, trusts ${pct(f.trusts)}, shares ${pct(f.shares)})`;
  };
...
// (c) self-consistency: the same message three times
{
  ...
  for (let i = 0; i < 100; i++) {
    for (let r = 1; r < runs.length; r++) {
      ...
      noulMoves.push(Math.abs(a.understands - b.understands), Math.abs(a.trusts - b.trusts), Math.abs(a.shares - b.shares));
    }
  }
  ...
  console.log(`    top reaction flipped in ${topFlips} of 200 face-transitions; noul absolute move mean ${mean(noulMoves).toFixed(3)}, max ${Math.max(...noulMoves).toFixed(3)}`);
...
// (d) sanity on hand-picked personas
{
  ...
  const show = (label: string, w: WallResponse, who: string) => {
    ...
    console.log(`    ${label.padEnd(9)} ${who.padEnd(18)} ${topReaction(f.reaction).padEnd(10)} ${pct(f.reaction[topReaction(f.reaction)]).padStart(4)}  understands ${pct(f.understands).padStart(4)}  trusts ${pct(f.trusts).padStart(4)}  shares ${pct(f.shares).padStart(4)}`);
  };
